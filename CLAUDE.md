...
기본값은 **Sonnet**. Linear에 `model/opus` 라벨이 붙은 티켓만 **Opus**로 돈다(`ty.py next`가 티켓마다 표시한다).

라벨을 붙이는 기준은 하나다 — **이 티켓이 다른 코드가 앞으로 지켜야 할 약속을 만드는가?**

| | 구조 → Opus | 구현 → Sonnet |
|---|---|---|
| 산출물 | 메시지 타입 · 시그니처 · DB 스키마 · config 포맷 · 상태 머신 · 평가 조건표의 숫자 | 노드 · 스크립트 · 뷰 · 리포트 · 테스트 |
| 성격 | 약속을 만든다 | 정해진 약속 안을 채운다 |

예외 하나: **동시성 · 실시간 타이밍처럼 테스트가 못 잡고 조용히 틀리는** 영역은 구현이어도 Opus다(큐 claim, 50Hz 루프, 서보 드라이버).

epic의 앞 한두 개가 구조고 나머지가 구현인 패턴이 반복된다. 큐가 epic당 하나씩 순서대로 내보내므로 구조 티켓이 먼저 끝나고 구현 티켓이 열린다.

**에스컬레이션은 없다.** 구현 티켓을 하다가 약속을 바꿔야 한다고 판단되면 바꾸지 말고 멈춘다 — 하위 이슈로 남기거나 `needs/decision`을 붙인다. Sonnet이 막혔다고 Opus가 자동으로 이어받지 않는다. 약속 변경은 사람 게이트다.

## 교차 검증은 한 번, 차단 기준은 좁게

구현한 세션이 스스로 통과를 선언하지 않는다. PR을 올린 뒤 **Sonnet 에이전트 하나가 1회 검증**한다(구조 티켓이어도 검증은 Sonnet).

차단은 네 가지뿐이다 — `unmet`(완료 조건 미충족) · `contract`(계약 위반) · `bug`(재현 경로를 말할 수 있는 결함) · `unverified`(수치 없음 또는 확인 명령이 안 돎). 네이밍 · 스타일 · 리팩터 제안 · 미래 확장성은 차단하지 않고 코멘트로만 남긴다. 기준을 넓히면 검증이 무한 루프가 된다.

`BLOCK`이면 지적된 것만 고쳐 재작업 1회. 재검증도 실패하면 `needs/decision`으로 사람에게 넘긴다. 세 번째 시도는 없다.

## 반드시 사람에게 넘기는 것

1. **`type/spike` 티켓과 설계 변경** — 태스크 정의 · 평가 조건표 · 인터페이스 계약의 숫자를 바꾸는 결정. 근거를 정리해 제시하고 멈춘다
2. **PR 머지** — 올리기까지만 한다. 단, 아래 자동 머지 조건을 **전부** 만족하면 `gh pr merge --auto --squash`로 맡긴다(CI가 통과해야 실제로 머지된다)
   - 티켓에 `model/opus` 라벨이 없다 (구현 티켓이다)
   - PR이 보호 경로를 건드리지 않는다 — `ros2_ws/src/ty_msgs/**` · `config/**` · `docs/interfaces.md` · `docs/task-pick-place.md` · `docs/design.md` · `platform/src/trainyard/evaluation/**` · DB 마이그레이션
   - 평가 수치를 바꾸지 않는다
   - CI 필수 검사가 ruleset에 걸려 있다 (그 전까지는 자동 머지를 쓰지 않는다)
   하나라도 어긋나면 사람이 머지한다
